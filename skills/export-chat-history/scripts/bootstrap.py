...
def _skill_root(script_dir: Path) -> Path:
    return script_dir.parent


def _venv_dir(script_dir: Path) -> Path:
    return _skill_root(script_dir) / ".venv"
...
def _deps_up_to_date(requirements_file: Path, venv_dir: Path) -> bool:
    ...
    return stamp.is_file() and stamp.read_text().strip() == _file_hash(
        requirements_file
    )
...
def _ensure_venv(venv_dir: Path, venv_python: Path) -> int:
    ...
    python_executable = sys.executable or next(
        (
