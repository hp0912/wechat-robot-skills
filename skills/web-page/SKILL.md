...
```json
{
  ...
  "properties": {
    ...
    "captcha_strategy": {
      "type": "string",
      "enum": ["detect", "ignore"],
      "description": "验证码处理策略。detect 表示执行动作前后检测疑似验证码并停止；ignore 表示不检测。传 actions 时默认 detect，否则默认 ignore。"
    }
  },
  "required": ["url"],
...
- `captcha_strategy` 只能是 `detect` 或 `ignore`。
- 自动化动作必须符合对应类型的必填字段要求，例如 `fill` 必须传 `selector` 和 `value`，`click` 必须传 `selector` 或 `text`。
